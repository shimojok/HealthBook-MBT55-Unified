...
def render_health_assessment():
    ...
    with tab1:
        ...
        if st.button(i18n.t("questionnaire.submit"), type="primary"):
            ...
            # PATHスコア
            if result.phenotype and result.phenotype.scores:
                st.markdown("### PATH_01〜05 代謝経路活性スコア")
                ...
                st.warning("スコアが計算できませんでした")

            # 疾病リスク
            if result.disease_risks:
                st.markdown("### ⚠️ 疾病リスク評価")
                for disease, risk in result.disease_risks.items():
                    st.write(f"- **{disease}**: {risk:.1f}%")
        ...
        if st.session_state.pipeline_result:
            ...
            for pid, ps in result.phenotype.scores.items():
                ...
                with col3:
                    ...
                    st.markdown(
                        f"<span style='color:{color}'>{ps.level}</span>",
                        unsafe_allow_html=True,
                    )
        else:
